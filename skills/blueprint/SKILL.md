...
diagrama", "con figma", "en pencil", "sin diagrama", "solo html" la reemplazan.
...
luego llama a `mcp__claude_ai_Figma__use_figma`. Traduce los patrones del blueprint
...
para crear un nuevo archivo. Establece los tokens de diagrama de Odissey mediante
`mcp__pencil__set_variables`. Luego usa `mcp__pencil__batch_design` para
insertar frames de lane, luego nodos dentro de cada lane, divisores discontinuos entre
niveles, y tarjetas de servicio en los lanes de backstage + support (con trazos discontinuos
para el nivel support).
