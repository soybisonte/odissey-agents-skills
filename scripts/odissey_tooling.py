...
OWNERSHIP_HEADER = "<!-- Generated by Odissey build; DO NOT EDIT. -->"
...
def normalize_newlines(text: str) -> str:
    """Return text with deterministic LF line endings."""
    return text.replace("\r\n", "\n").replace("\r", "\n")


def atomic_write_text(path: Path, text: str) -> None:
    """Atomically replace ``path`` with normalized UTF-8 text."""
    path = Path(path)
    ...
    try:
        with os.fdopen(descriptor, "w", encoding="utf-8", newline="\n") as temporary_file:
            temporary_file.write(normalize_newlines(text))
